...
export default class Condition {
   ...
   */
  private static _getConditionType (condition) {
    ...
    if (condition === Object(condition)) {
      if (Object.prototype.hasOwnProperty.call(condition, PROPS.evaluator)) {
   ...
   */
  private static _checkPrimitive (condition, testValue) {
  ...
  private static _checkRange (element, testValue) {
    ...
    if (typeof element !== 'string') {
      throw new Error(
        'Expected a string type of element for range check: ' + element
      )
    }

    if (typeof testValue !== 'number' || isNaN(testValue)) {
      throw new Error(
        'Expected a numerical type of context value for range check: ' +
          testValue
      )
    }
   ...
   */
  private static _checkCustomEvaluator (
    condition,
    testValue,
    customEvaluators
  ) {
