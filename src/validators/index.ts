          ...
          if (
            ...
            errors.push({
              message:
                ...
                ', your dependency `' +
                dependencyName +
                '` was referenced before it was resolved. Move the definition of it above ' +
                entry.setting
            })
          }
        }
...
function _validateTemplates (config) {
  ...
  config.forEach(function (entry) {
    if (isTemplate(entry.value)) {
      errors.push({
        message:
          'Setting `' + entry.setting + '` has template as toplevel value'
      })
    }
    ...
    if (entry.except && Array.isArray(entry.except)) {
      entry.except.forEach(function (exceptBlock) {
        ...
        if (variableMatches.length > 1) {
          ...
          if (!Object.prototype.hasOwnProperty.call(exceptBlock, condition)) {
            errors.push({
...
function _checkValueTypes (value, expectedType, errors?) {
  ...
  var pattern
  var regexStr
  var valType = Array.isArray(value) ? 'array' : typeof value
  ...
  switch (expectedType) {
    ...
    default:
      if (typeof expectedType === 'object') {
        isRegex = Object.prototype.hasOwnProperty.call(expectedType, 'regex')

        if (
          isRegex ||
          Object.prototype.hasOwnProperty.call(expectedType, 'iregex')
        ) {
          if (isRegex) {
